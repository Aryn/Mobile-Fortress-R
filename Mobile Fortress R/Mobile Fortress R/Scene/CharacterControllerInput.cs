...
namespace Mobile_Fortress_R
{
    ...
    /// </summary>
    class CharacterControllerInput : IControllable
    {
        /// <summary>
        ...
        public void Update(float dt)
        {
            if (IsActive)
            {
                ...
                CharacterController.WorldTransform = Camera.World;

                if (MobileFortress.Controller.IsKeyDown(Keys.Q))
                    CharacterController.Body.LinearVelocity += Camera.World.Forward;
                if (MobileFortress.Controller.IsKeyDown(Keys.E))
                    CharacterController.Body.LinearVelocity += Vector3.Up * dt * PhysicsPiece.Gravity;
            }
        }



        public void ControlUpdate(IController controller)
        {
            ...
            if (controller.IsKeyDown(CharacterController.forward))
            {
                movementDir = Camera.World.Forward;
                totalMovement += Vector2.Normalize(new Vector2(movementDir.X, movementDir.Z));
            }
            if (controller.IsKeyDown(CharacterController.backward))
            {
                movementDir = Camera.World.Forward;
                totalMovement -= Vector2.Normalize(new Vector2(movementDir.X, movementDir.Z));
            }
            if (controller.IsKeyDown(CharacterController.left))
            {
                movementDir = Camera.World.Left;
                totalMovement += Vector2.Normalize(new Vector2(movementDir.X, movementDir.Z));
            }
            if (controller.IsKeyDown(CharacterController.right))
            {
            ...
            CharacterController.StanceManager.DesiredStance = controller.IsKeyDown(CharacterController.crouch) ? Stance.Crouching : Stance.Standing;
            //Jumping
            if (controller.WasKeyPressed(CharacterController.jump))
            {
        ...
        public void TakeControl(IController controller)
        {
        }

        public void ReleaseControl(IController controller)
        {
        }
    }
}