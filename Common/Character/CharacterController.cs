...
namespace Common.Character
{
    ...
    public class CharacterController : Updateable, IBeforeSolverUpdateable
    {
        ...
        public const Keys crouch = Keys.LeftShift;
        ...
        public CharacterController()
            : this(new Vector3(), 6.2f, 6.2f * .7f, .6f, 7)
        {
        ...
        public CharacterController(Vector3 position, float height, float crouchingHeight, float radius, float mass)
        {
            ...
            SupportFinder.MaximumSlope = 0.61f;
        ...
        public bool tryToStep { get { return Body.Position.Y > Sector.seaLevel; } }
        ...
        void IBeforeSolverUpdateable.Update(float dt)
        {
            ...
            try
            {
                ...
                if (relativeVelocity.LengthSquared() > 15 * 15) SupportFinder.MaximumSlope = 0.35f;
                else SupportFinder.MaximumSlope = 0.61f;
                ...
                if (Body.Position.Y < Sector.seaLevel)
                {
                    this.ChangeVelocityUnilaterally(Vector3.Up * (PhysicsPiece.GetBuoyantForces(Body, dt) / Body.Mass), ref relativeVelocity);
                }
                if (Body.LinearVelocity.LengthSquared() > 0.1f)
                {
                    Vector3 v = -Vector3.Normalize(Body.LinearVelocity) * (PhysicsPiece.GetDrag(Body, dt, 2f) / Body.Mass);
                    this.ChangeVelocityUnilaterally(v, ref relativeVelocity);
                }
                //Try to step!
                Vector3 newPosition;
                if (tryToStep)
        ...
        public void LerpToPosition(Vector3 newPosition, float dt)
        {
            if ((newPosition - Body.Position).LengthSquared() < 5 * 5)
            {
                Vector3 pos = Vector3.Lerp(Body.Position, newPosition, 0.2f);
                TeleportToPosition(pos, dt);
            }
            else TeleportToPosition(newPosition, dt);
        }
